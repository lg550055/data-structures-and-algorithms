...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort((a, b) => a.height*1 > b.height*1 ? -1 : 1);
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(e =>
    result.push(e.slice(e.indexOf(' ',3)+1)));
  return result;
};
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(e =>
    result.push(e.split(' ').slice(2).reduce((p,c)=>p+' '+c)));
  return result;
};
...
const totalSumCSV = (str) => {
  let total = 0;
  total = str.split(',').reduce((p,c) => p*1+c*1)*1;
  return total;
};
...
const removeVowels = (str) => {
  const v = ['a','e','i','o','u'];
  return [...str].reduce((p,c) => v.includes(c) ? p : p+c, '');
};
