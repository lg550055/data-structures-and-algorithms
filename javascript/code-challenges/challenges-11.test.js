...
function transformToLis(obj){
  return Object.entries(obj).map(e => '<li>'+e[0]+': '+e[1]+'</li>');
}
...
const count = (target, input) => {
  return input.reduce((p,c) => p+ c.reduce((a,b)=> b===target ? a+1 : a,0),0 );
};
...
const totalSum = (input) => {
  return input.reduce((p,c) => p+ c.reduce((a,b)=> a+b,0),0 );
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(e => e.filter(x =>Number.isInteger(x) && x%5===0).map(y => Math.pow(2,y)));
};
...
let findMaleAndFemale = (data) => {
  return data.filter(e => e.gender.includes('male')).reduce((p,c)=> p ? p+' and '+c.name : c.name,'');
};
...
let findShortest = (data) => {
  let minH = data.reduce((p,c) => Math.min(c.height, p), data[0].height);
  return data.filter(e => e.height == minH)[0].name;
};
