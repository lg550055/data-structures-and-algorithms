...
const getStatName = (arr, minBaseStat) => {
  const filtered =  arr.filter(e => e.baseStat > minBaseStat);
  return filtered.map(e => e.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  const filtered = arr.filter(e => typeof e === 'number');
  return filtered.map(e => e%2 ? 'odd' : 'even');
};
