...
const screenForNames = (arr) => {
  return arr.filter(n => /^[MD][rs]s?\.\s[A-Z]\w+/.test(n));
};
...
let biggerThanLuke = (arr) => {
  let result = '';
  // let m = arr.filter(e => e.name.includes('Luke'))[0].mass*1;
  arr.forEach(e => {
    if (e.mass*1 > 77) {result = result + e.name + ' - '}
  });
...
const detectTicTacToeWin = (board) => {
  let helpCheck = a => board[a[0]][a[1]] && board[a[0]][a[1]] === board[a[2]][a[3]] && board[a[0]][a[1]] === board[a[4]][a[5]] ? true : false; 

  const lines = [[0,0,0,1,0,2],[1,0,1,1,1,2],[2,0,2,1,2,2],[0,0,1,0,2,0],[0,1,1,1,2,1],[0,2,1,2,2,2],[0,0,1,1,2,2],[2,0,1,1,0,2]];

  for(let i=0; i<8; i++) {
    if (helpCheck(lines[i])) {return true}
  }
