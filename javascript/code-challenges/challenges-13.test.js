...
const longestString = (arr) => {
  return arr.indexOf(arr.reduce((p,c) => c.length > p.length ? c : p, ''));
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(s => s.substring(1,4)+s.substring(6,9)+s.substring(10));
};
...
const allHappy = (arr) => {
  return arr.reduce((p,c) => /:\)/.test(c) && p, true);
};
...
const findEvery = (arr, target) => {
  return arr.reduce((p,c) => c.includes(target) && p, true);
};
...
const unenrollBrook = (arr) => {
  return arr.map(e => e.filter(s => !s.includes('Brook')));
};
